defmodule Credits.Application do
  ...
  def start_normal() do
    nats_config = Application.get_env(:credits, :nats)

    children = [
      # Start the MongoDB connection supervisor
      {Mongo, Mongo.config()},
          ...
          {Gnat.ConnectionSupervisor, :start_link,
           [
             %{
               ...
               connection_settings: [nats_config[:connection_settings]]
             }
           ]}
      },

      # Start the Broadway pipeline
      {Events.Broadway, gnat: nats_config[:name], subscriptions: nats_config[:subscriptions]}
    ]
